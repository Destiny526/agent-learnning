...
import type { Ticket, PreferenceWeights } from '@/lib/mock-data';
...
export default function SearchResultCard({ ticket, date, index = 0, weights }: Props) {
  ...
  const totalSeats = ticket.seatClasses.reduce((s, c) => s + c.available, 0);

  return (
    <div
      onClick={handleClick}
      className="group bg-white rounded-2xl border border-gray-100 overflow-hidden cursor-pointer transition-all duration-300 hover:shadow-xl hover:shadow-gray-200/60 hover:border-gray-200 hover:-translate-y-1 animate-fade-in-up"
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 mb-4">
              <div className="w-11 h-11 rounded-xl bg-gray-50 flex items-center justify-center flex-shrink-0">
                <span className="text-xl">{ticket.typeIcon}</span>
              </div>
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-gray-400">{ticket.carrier}</span>
                </div>
                <div className="flex items-center gap-2 text-xs text-gray-500 mt-0.5">
                  <span>{ticket.seatClasses[ticket.seatClasses.length - 1]?.class}</span>
                  <span className="text-gray-300">·</span>
                  <span>余 {totalSeats} 张</span>
                  <span className="text-gray-300">·</span>
                  <span>{ticket.punctuality}% 准点</span>
                </div>
              </div>
            </div>
          ...
          <div className="lg:w-64 flex-shrink-0 flex flex-row lg:flex-col items-center lg:items-end gap-5 lg:gap-4 pt-0 lg:pt-2 border-t lg:border-t-0 lg:border-l border-gray-100 lg:pl-6">
            ...
            <div className="flex flex-col gap-2 w-full lg:w-auto">
              <button
                ...
                className="w-full lg:w-44 px-5 py-3 bg-gradient-to-r from-primary-600 to-primary-500 text-white font-semibold rounded-xl hover:from-primary-700 hover:to-primary-600 hover:shadow-lg hover:shadow-primary-600/25 active:scale-[0.97] transition-all text-sm text-center"
              >
                查看详情
              </button>
              <button
